...
get_tooltip() {
  ...
  local self_ip=$(echo "$status_json" | jq -r '.Self.TailscaleIPs[] | select(test("^100\\."))')

  # Build tooltip with hostname and peers using Pango markup
  local tooltip="<b>Hostname: <span foreground='#87CEEB'>$hostname</span> <span foreground='#87CEEB'>($self_ip)</span></b>\n"

  tooltip+="\nPeers:"

  # Get peers and their status
  local peers=$(echo "$status_json" | jq -r '.Peer // {} | to_entries[] | "\(.value.DNSName | split(".")[0]):\(.value.TailscaleIPs[] | select(test("^100\\."))):\(.value.Online)"' | sort)

  if [[ -n "$peers" ]]; then
    while IFS=: read -r peer_name peer_ip peer_online; do
      if [[ "$peer_online" == "true" ]]; then
        tooltip+="\n<span foreground='#00ff00'>●</span> $peer_name ($peer_ip)"
      else
